    ...
    <b-row class="my-12">
      <b-col cols="12">
        ...
        <b-form @submit="register">
          <b-form-group id="input-group-1" label="用户名:" label-for="uname" description="请输入用户名">
            <b-form-input
              id="uname"
              v-model="form.uname"
              type="text"
...
export default {
  ...
  methods: {
    ...
    checkUname() {
      this.$axios
        .post("/user/check_uname", qs.stringify({ uname: this.form.uname }))
        .then(res => {
          console.log(res);
    ...
    checkPhone() {
      this.$axios
        .post("/user/check_phone", qs.stringify({ phone: this.form.phone }))
        .then(res => {
          console.log(res);
          if (res.data.code == 301) {
            alert("手机号已存在,请换一个");
  ...
  computed: {
    ...
    validatePhone() {
      if (this.form.phone != "") {
        var pt = /^1[3-9]\d{9}$/;
        return true && pt.test(this.form.phone);
      } else {
        return false;
      }
    }
